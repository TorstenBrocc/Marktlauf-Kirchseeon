-- Bestätigungs-Standard: Backticks um {{firma}} in Abschnitt 6 entfernt (Master 1:1 aus
-- sponsorBriefDefaults()['bestaetigung']) UND die alten allgemeinen persönlichen Entwürfe
-- (sponsor_id=0) von TT (user 3) + Anja (user 4) entfernt, damit der neue Master greift
-- (Inhaber-Freigabe 2026-08-20). Pro-Sponsor-Entwürfe (sponsor_id>0) bleiben unberührt.
...
DELETE FROM briefvorlagen_entwurf
 WHERE vorlage_art = 'sponsor' AND slug = 'bestaetigung' AND sponsor_id = 0 AND user_id IN (3, 4);
